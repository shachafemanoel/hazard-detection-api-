...
class RootResponse(BaseModel):
    ...
    endpoints: List[str] = Field(..., example=["/health", "/status", "/session/start", "/detect"])
...
class Detection(BaseModel):
    ...
    bbox: List[float] = Field(..., description="Bounding box [x1, y1, x2, y2]", example=[100.0, 100.0, 200.0, 200.0])
    confidence: float = Field(..., description="Detection confidence score", example=0.85, ge=0.0, le=1.0)
    class_id: int = Field(..., description="Class ID", example=0)
    class_name: str = Field(..., description="Class name", example="Pothole")
    center_x: float = Field(..., description="Bounding box center X coordinate", example=150.0)
    center_y: float = Field(..., description="Bounding box center Y coordinate", example=150.0)
    ...
    is_new: Optional[bool] = Field(None, description="Whether this is a new detection (session-based)")
    report_id: Optional[str] = Field(None, description="Associated report ID if generated")
...
class DetectionReport(BaseModel):
    ...
    session_id: str = Field(..., example="123e4567-e89b-12d3-a456-426614174000")
    detection: Detection = Field(..., description="Detection that generated this report")
    timestamp: str = Field(..., example="2024-08-06T10:30:00")
    status: str = Field(..., example="pending", description="Report status: pending, confirmed, dismissed")
    location: Dict[str, Any] = Field(..., example={
        "bbox": [100, 100, 200, 200],
        "center": [150, 150]
    })
    frame_info: Dict[str, Any] = Field(..., example={
        "has_image": True,
        "image_size": 1024
    })
...
class SessionSummary(BaseModel):
    ...
    reports: List[DetectionReport] = Field(..., description="All reports in this session")
...
class DetectionResponse(BaseModel):
    ...
    new_reports: Optional[List[DetectionReport]] = Field(None, description="New reports generated")
    session_stats: Optional[SessionStats] = Field(None, description="Session statistics")
    processing_time_ms: float = Field(..., example=150.5, description="Processing time in milliseconds")
...
class BatchDetectionResponse(BaseModel):
    ...
    success: bool = Field(..., example=True)
    results: List[BatchDetectionResult] = Field(..., description="Results for each file")
    total_processing_time_ms: float = Field(..., example=450.2, description="Total processing time")
    processed_count: int = Field(..., example=3, description="Number of files processed")
    successful_count: int = Field(..., example=2, description="Number of successfully processed files")
    model_info: ModelInfo = Field(..., description="Model information")
...
class DeviceInfo(BaseModel):
    ...
    input_shape: Optional[List[int]] = Field(None, example=[1, 3, 512, 512])
    output_shape: Optional[List[int]] = Field(None, example=[1, 25200, 15])
    model_path: Optional[str] = Field(None, example="best0408_openvino_model/best0408.xml")
...
class ModelFiles(BaseModel):
    ...
    openvino_model: str = Field(..., example="/app/best0408_openvino_model/best0408.xml")
...
class ErrorDetail(BaseModel):
    ...
    additional_info: Optional[Dict[str, Any]] = Field(None, description="Additional error context")