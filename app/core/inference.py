...
class ModelInference:
    def __init__(self, model_xml_path: str = "./best0408_openvino_model/best0408.xml", device_name: str = "CPU"):
        self.class_names = ['crack', 'pothole']  # Adjust if necessary
        try:
            core = Core()
            ...
            print(f"✅ Model '{model_xml_path}' loaded successfully.")
    ...
    def _postprocess(self, results: np.ndarray, scale: float, pad_dx: int, pad_dy: int, conf_threshold=0.5):
        detections = []
        for box in results[0]:
            confidence = box[4]
            if confidence >= conf_threshold:
                class_id = int(box[5])
                x1 = int((box[0] - pad_dx) / scale)
                y1 = int((box[1] - pad_dy) / scale)
                x2 = int((box[2] - pad_dx) / scale)
                y2 = int((box[3] - pad_dy) / scale)
                detections.append({
