...
def test_confirm_report_not_found_session(client: TestClient):
    ...
    response = client.post(f"/session/{fake_session_id}/report/{fake_report_id}/confirm")
    assert response.status_code == 404
...
def test_dismiss_report_not_found_session(client: TestClient):
    ...
    response = client.post(f"/session/{fake_session_id}/report/{fake_report_id}/dismiss")
    assert response.status_code == 404
