...
class ModelService:
    ...
    async def load_model(self) -> bool:
        ...
        if settings.model_backend in ['openvino', 'auto']:
            if await self._try_load_openvino():
                ...
                raise ModelLoadingException("OpenVINO backend requested but loading failed")
        
        if settings.model_backend in ['pytorch', 'auto']:
            if await self._try_load_pytorch():
                return True
            elif settings.model_backend == 'pytorch':
                raise ModelLoadingException("PyTorch backend requested but loading failed")
    ...
    async def _try_load_openvino(self) -> bool:
        ...
        try:
            ...
            if input_info.partial_shape.is_dynamic:
                ...
                new_shape = ov.PartialShape([1, 3, settings.model_input_size, settings.model_input_size])
            ...
            compiled_model = core.compile_model(model=model, device_name=settings.openvino_device, config=config)
                ...
                for path in temp_config.pytorch_model_paths:
                    if path.exists():
                        try:
                            ...
                            self.backend = "pytorch"
                            self.is_loaded = True
                            logger.info("🚀 Using PyTorch backend from fallback location")
                            return
                        except Exception:
                            continue
    ...
    async def _predict_openvino(self, image: Image.Image) -> List[DetectionResult]:
        ...
        processed_image, scale, paste_x, paste_y = self._preprocess_image(image, input_shape)
        
        # Run inference
        if settings.openvino_async_inference and self.infer_request:
            self.infer_request.infer(inputs={self.input_layer.any_name: processed_image})
            result = self.infer_request.get_output_tensor(self.output_layer.index).data
        else:
            result = self.compiled_model({self.input_layer.any_name: processed_image})[self.output_layer]
        
        # Postprocess results
        return self._postprocess_predictions(
            result, image.width, image.height, input_shape[3], input_shape[2],
    ...
    def _preprocess_image(self, image: Image.Image, input_shape: List[int]) -> Tuple[np.ndarray, float, int, int]:
    ...
    def _postprocess_predictions(self, predictions: np.ndarray, original_width: int, original_height: int,
                                input_width: int, input_height: int, scale: float, paste_x: int, paste_y: int) -> List[DetectionResult]:
